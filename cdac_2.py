"""
- 不涉及到三元组损失, 但是能监控相似度矩阵分布、统计样本对数量(实现可能有问题)
- 其实包含两种实现, 遵循原作者的实现, 损失函数在model里内置; 其次就是结合我自己写的函数
- 模型调用应该改成BertTest类, 已改
"""
...
class CdacManager:
    ...
    def __init__(self, args, data_processor, logger_name='Main Training'):
        ...
        self.model = BertTest(model=args.model, feat_dim=args.feat_dim).to(self.device)
